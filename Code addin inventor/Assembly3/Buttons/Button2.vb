...
Namespace ToolInventor2025.Assembly3.Buttons
    Public Module Button2
        Public Sub OnExecute(ByVal Context As NameValueMap)


            Try
                NumberAssemblyStockNumber()
            Catch ex As Exception
                MessageBox.Show("Error running NumberAssemblyStockNumber: " & ex.Message, "Assembly2 - Stock Number")
            End Try
        End Sub

        Private Sub NumberAssemblyStockNumber()
            Dim asmDoc As AssemblyDocument = TryCast(g_inventorApplication.ActiveDocument, AssemblyDocument)
            If asmDoc Is Nothing Then
                MessageBox.Show("Active document is not an assembly.", "Assembly2 - Stock Number")
                Return
            End If
            ...
            Dim prefix As String = Microsoft.VisualBasic.Interaction.InputBox("Nhập prefix cho STT Stock Number (ví dụ: TH, để trống nếu không dùng):", "Prefix STT Stock Number", "")
            prefix = prefix.Trim()
            ...
            ' Write Stock Number for all documents in each group (apply prefix to Stock Number)
            For Each kvp As KeyValuePair(Of String, String) In partKeyToSTT
                ...
                If partKeyToDocs.ContainsKey(key) Then
                    For Each doc As Document In partKeyToDocs(key)
                        SetStockNumber(doc, fullSTT)
                    Next
                End If
            Next

            MessageBox.Show("Hoàn tất: Structured BOM (top-level only, không prefix) và Model Data (Stock Number với prefix '" & prefix & "') đã được đánh STT.", "iLogic - Converted")
        End Sub
        ...
        Private Sub SetStockNumber(doc As Document, value As String)
            Try
                Dim ps As PropertySet = doc.PropertySets.Item("Design Tracking Properties")
                Dim prop As [Property] = Nothing
                Try
                    prop = ps.Item("Stock Number")
                Catch
                End Try
                If prop Is Nothing Then
                    ' If property doesn't exist, attempt to add it via SummaryInfo if supported
                    Try
                        ps.Add("Stock Number", value)
                    Catch
                        ' ignore add failures
                    End Try
                Else
